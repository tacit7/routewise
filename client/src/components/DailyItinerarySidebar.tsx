...
import { Calendar, Clock, GripVertical, Star, Trash2, MapPin, Plus, Timer, FileText } from "lucide-react";
...
export default function DailyItinerarySidebar({
  ...
  return (
    <Card className="w-[450px] h-full rounded-none border-0 border-r">
      ...
      <CardContent className="p-4">
        <div
          className="min-h-32 border-2 border-solid rounded-lg p-4 transition-colors bg-bg border-border"
        >
          {sortedPlaces.length === 0 && (
            <div className="text-center py-8">
              <Clock className="h-8 w-8 text-muted-foreground mx-auto mb-2" />
              <p className="text-sm text-muted-foreground">No places scheduled for this day</p>
              <p className="text-xs text-muted-foreground mt-1">Add places using the grid on the right</p>
            </div>
          )}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <Input
                            type="text"
                            placeholder="2 hours"
                            value={(place as any).duration || ""}
                            onChange={(e) => onPlaceUpdate?.(getIdentifier(place), { duration: e.target.value })}
                        ...
                        <div className="space-y-1">
                          ...
                          <Textarea
                            ...
                            value={place.notes || ""}
                            onChange={(e) => onPlaceUpdate?.(getIdentifier(place), { notes: e.target.value })}
